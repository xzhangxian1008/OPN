...
class RS_MLP(nn.Module):
    def __init__(self, Output_Dim, Dynamic_Emb_Num):
        ...
        self.attention = nn.Sequential(
            nn.Linear(fixed_emb_size, fixed_emb_size * 2),
            nn.BatchNorm1d(fixed_emb_size * 2),
            nn.Linear(fixed_emb_size * 2, fixed_emb_size)
        )
        ...
        self.transfrom = nn.Sequential(
            ...
            nn.BatchNorm1d(fixed_emb_size * 2),
            nn.Tanh(),
            nn.Linear(fixed_emb_size * 2, Output_Dim)
        )
        # self.transfrom = nn.Sequential(
        #     nn.BatchNorm1d(fixed_emb_size * 2),
    ...
    def forward(self, userId, movieId, movie_vec):
        ...
        v_cross = v_user * v_movie

        weights = self.attention(v_cross)
        weights = self.softmax(weights)
        v_cross = v_cross * weights

        # user_movie = torch.cat((v_user, v_movie), 1)
        return self.transfrom(v_cross)
